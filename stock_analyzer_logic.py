...
def analyze_ticker(ticker):
    ...
    for tf_name in ["4h", "1d", "1wk", "1mo"]:
        ...
        ref_price = df.iloc[-1]['close']
        sl = cross_price * (1 - SL_BUFFER if status == "Uptrend" else 1 + SL_BUFFER)
        tp_price, tp_note = find_previous_opposing_cross(df, status, ref_price)
        if tp_price is None: tp_price, tp_note = find_next_sr_level(ticker, tf_name, status, ref_price)
        ...
        # Build technical trace
        current_trace = " | ".join(log_trace + [f"{tf_name}:{bars_ago if bars_ago is not None else 'Act'}"])

        result = {
            "Ticker": ticker, "Signal": f"{'CONFIRMED' if is_fresh else 'EXISTING'} {status.upper()}",
            "Timeframe": tf_name, "Stop Loss": round(sl, 4), "Take Profit": f"{tp_price} ({tp_note})",
            "Exit Warning": check_early_exit(ticker, tf_name, status), "Cross Time": cross_str,
            "Bar Trace": current_trace
        }
    ...
    for ticker in tickers:
        try:
            ...
            if res.get("Signal") != "No Signal":
                res["Eco Calendar"] = danger
                results.append(res)
            else:
                results.append({"Ticker": ticker, "Signal": "No Signal", "Eco Calendar": danger, "Bar Trace": res.get("Bar Trace", "-")})
        except Exception as e:
            results.append({"Ticker": ticker, "Signal": "Error", "Eco Calendar": str(e)})
